...
# Hugging Face MiniLM intent detection
...
def chatbot_response(
    ...
    # Use Hugging Face MiniLM for semantic intent detection.
    # A confidence threshold prevents unrelated questions from
    # being forced into one of the known SterliFlux intents.
    if intent is None and q:
        try:
            detected_intent, intent_score = detect_intent(q)
            intent = (
                detected_intent
    ...
    if not q:
        return (
            "Ask me about your spending, savings, forecasts, "
            "financial health, investments, affordability, "
            "or inflation for a specific category."
        )
    ...
    if intent == "greeting" or q in (
        ...
        return (
            ...
            "You can ask me about your spending, savings, "
            "financial health, forecasts, affordability, "
    ...
    if q in (
        ...
        return (
            "You're welcome! I'm here whenever you want "
    ...
    if q in (
        ...
        return (
            "Sure. If you need anything else, you can ask "
            "me about your spending, savings, forecasts, "
            "financial health, or affordability."
        )

    # -----------------------------------------------------
    # AFFORDABILITY - PURCHASE AMOUNT
    # -----------------------------------------------------
    #
    # Extract an amount from questions such as:
    # "Can I afford a 50000 purchase?"
    # "Can I afford ₹50,000?"
    # "Can I buy something for Rs 20,000?"
    #
    purchase_keywords = (
        "afford",
        "buy",
    ...
    if is_purchase_question:
        ...
        if not amount_match:
            amount_match = re.search(
                r"\b([\d,]+(?:\.\d+)?)\s*"
                r"(?:purchase|expense|spend|payment|item)\b",
                q,
                re.IGNORECASE
            )
        ...
        if amount_match:
            ...
            if purchase_amount is not None:
                ...
                # Compare against predicted savings.
                if predicted_savings >= purchase_amount:
                    remaining = (
                        predicted_savings
                        - purchase_amount
                    )
                    ...
                    return (
                        f"Based on your current forecast, "
                        f"you could afford a ₹"
                        f"{purchase_amount:,.2f} purchase. "
                        f"Your predicted next-month savings "
                        f"are ₹{predicted_savings:,.2f}, "
                        f"leaving approximately ₹"
                        f"{remaining:,.2f} after the purchase. "
                        f"However, consider keeping an emergency "
                        f"buffer before making a large purchase."
                    )
                    ...
                    return (
                        f"A ₹{purchase_amount:,.2f} purchase "
                        f"would be higher than your predicted "
                        f"next-month savings of ₹"
                        f"{predicted_savings:,.2f}. "
                        f"You would be short by approximately ₹"
                        f"{shortfall:,.2f}. "
                        f"I'd recommend postponing it or "
                        f"reducing the purchase amount unless "
                        f"you already have sufficient savings "
                        f"outside this forecast."
                    )

                else:
                    return (
                        f"I wouldn't recommend a ₹"
                        f"{purchase_amount:,.2f} purchase right now. "
                        f"Your predicted next-month savings are "
                        f"₹{predicted_savings:,.2f}, meaning your "
                        f"projected expenses already exceed your "
                        f"monthly income."
                    )
    ...
    if (
        ...
        if income <= 0:
            return (
                "I can't reliably assess house affordability "
                "because your monthly income is not available."
            )

        if predicted_savings <= 0:
            return (
                "Based on your current forecast, buying a house "
                "would be financially difficult right now because "
                "your projected monthly savings are not positive. "
                "Reducing expenses and building a stable savings "
                "buffer would be a better first step."
            )

        return (
            f"Based on your current forecast, you could potentially "
            f"start planning for a house, but SterliFlux cannot "
            f"confirm mortgage eligibility from expense data alone. "
            f"Your projected monthly savings are approximately ₹"
            f"{predicted_savings:,.2f}, which is about ₹"
            f"{annual_savings:,.2f} per year. "
            f"A lender would also consider your existing savings, "
            f"down payment, loan amount, interest rate, existing "
            f"debts, and credit profile. "
            f"For a major purchase like a house, keeping an "
            f"emergency fund in addition to the down payment is "
            f"recommended."
        )
    ...
    # -----------------------------------------------------
    if (
        "spending too much" in q
        or "spend too much" in q
        or "am i spending too much" in q
        or "overspending" in q
        or "over spending" in q
        or "spending a lot" in q
        or "spending alot" in q
        or "too much money" in q
    ):
        if income <= 0:
            return (
                f"Your predicted next-month expense is ₹"
                f"{next_month_expense:,.2f}. "
                f"I can't compare this with your income because "
                f"your monthly income is not available."
            )
        ...
        if expense_ratio > 100:
            return (
                f"Yes. Your predicted next-month spending is "
                f"₹{next_month_expense:,.2f}, which is about "
                f"{expense_ratio:.1f}% of your monthly income "
                f"of ₹{income:,.2f}. "
                f"That means your projected expenses exceed "
                f"your income. Reducing discretionary spending "
                f"should be a priority."
            )

        if expense_ratio > 80:
            return (
                f"Your spending is relatively high. Your predicted "
                f"next-month expense is ₹{next_month_expense:,.2f}, "
                f"around {expense_ratio:.1f}% of your income. "
                f"That leaves approximately ₹"
                f"{predicted_savings:,.2f} for savings. "
                f"Reducing non-essential expenses could improve "
                f"your financial buffer."
            )

        if expense_ratio > 60:
            return (
                f"Your predicted spending is ₹"
                f"{next_month_expense:,.2f}, around "
                f"{expense_ratio:.1f}% of your income. "
                f"You're not necessarily overspending, but there "
                f"is room to improve your savings by controlling "
                f"discretionary expenses."
            )

        return (
            f"Your predicted next-month spending is ₹"
            f"{next_month_expense:,.2f}, around "
            f"{expense_ratio:.1f}% of your income. "
            f"Based on this forecast, your spending appears "
            f"manageable and you have a reasonable savings margin."
        )
    ...
    if (
        ...
        or "category should i reduce" in q
        or "category should i cut" in q
        or "reduce category" in q
        or "cut category" in q
        or "where should i reduce" in q
        or "where should i cut" in q
        or "reduce my spending" in q
        or "cut my spending" in q
        or "reduce expenses" in q
        or "cut expenses" in q
    ):
        categories = data.get(
            "category_predictions",
            []
        ...
        if not categories:
            return (
                "I don't have category-level spending data "
                "to determine which category you should reduce."
            )

        # Find the largest category by historical spending.
        ...
        return (
            ...
            f"{category_name} is the first category I'd "
            f"review. It accounts for approximately "
            f"{category_percentage:.1f}% of your recorded "
            f"spending, with historical spending of about ₹"
            f"{category_total:,.2f}. "
            f"Its predicted next-month spending is around ₹"
            f"{category_prediction:,.2f}. "
            f"Reducing non-essential spending in this category "
            f"could have the biggest impact on your overall budget."
        )
    ...
    if (
        "invest" in q
        or "investment" in q
        or "where can i invest" in q
        or "where should i invest" in q
        or "what can i invest" in q
        or "how should i invest" in q
    ):
        if predicted_savings <= 0:
            return (
                "Before investing, I'd recommend focusing on "
                "stabilising your monthly cash flow. Your current "
                f"predicted savings are ₹{predicted_savings:,.2f}. "
                "Once you consistently have positive savings, "
                "consider building an emergency fund first."
            )

        monthly_investable = (
            predicted_savings * 0.20
        )

        return (
            f"Your forecast suggests approximately ₹"
            f"{predicted_savings:,.2f} of monthly savings. "
            f"Rather than investing all of it, you could consider "
            f"starting with a modest portion, such as around ₹"
            f"{monthly_investable:,.2f}, after maintaining an "
            f"emergency fund. "
            f"Depending on your goals and risk tolerance, common "
            f"options to research include diversified index mutual "
            f"funds, recurring deposits, fixed deposits, and "
            f"government-backed savings products. "
            f"SterliFlux cannot determine the best investment for "
            f"you or guarantee returns, so the choice should also "
    ...
    # OVERALL INFLATION
    # -----------------------------------------------------
    if intent == "inflation" or "inflation" in q:
        rate = data.get(
            "inflation_rate"
        )
    ...
    # -----------------------------------------------------
    #
    # RAG is used for general financial education questions
    # that are not answered by SterliFlux's deterministic
    # financial calculations.
    #
    # Personal financial predictions continue to use the
    # existing SterliFlux calculations above.
        ...
        try:
            ...
            if rag_result:
                ...
                return (
                    ...
                    "the SterliFlux financial knowledge "
                    "assistant."
                )
    ...
    return (
        ...
        "health, monthly/6-month/1-year forecasts, inflation, "
        "investment planning, spending reduction, affordability, "
        "and general financial concepts. Try asking something "
        "like 'What is an emergency fund?' or "
    ...
    if intent == "savings" or "saving" in q or "savings" in q:
    ...
    if intent == "income" or "income" in q or "earn" in q:
...
def chat():
    ...
    # Detect the user's intent with Hugging Face MiniLM.
    # Keep a conservative threshold so unrelated questions
    # continue to use the existing chatbot fallback.
    intent = "unknown"
    if question:
        try:
            detected_intent, intent_score = detect_intent(question)
            if intent_score >= 0.55:
